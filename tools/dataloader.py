...
class PairLoader:
    ...
    distort: random ditortion applied to img2
    ...
    def __getitem__(self, i):
        ...
        # print("img_a: ", type(img_a), np.min(img_a), np.max(img_a), np.median(img_a), img_a.shape)
        # print("img_b: ", type(img_b), np.min(img_b), np.max(img_b), np.median(img_b), img_b.shape)
        # exit(0)
        # gray_img_a = np.dot(img_a.astype(np.float)[..., :3], [0.229, 0.587, 0.114])
        # gray_img_b = np.dot(img_b.astype(np.float)[..., :3], [0.229, 0.587, 0.114])
        gray_img_a = img_a.transpose((2, 0, 1)).astype(np.float) / 255.
        gray_img_b = img_b.transpose((2, 0, 1)).astype(np.float) / 255.

        # print("grayimg_a1: ", type(gray_img_a), np.min(gray_img_a), np.max(gray_img_a), np.median(gray_img_a),
        #       gray_img_a.shape)
        # print("grayimg_b1: ", type(gray_img_b), np.min(gray_img_b), np.max(gray_img_b), np.median(gray_img_b),
        #       gray_img_b.shape)
        # exit(0)

        gray_img_a = 0.299 * gray_img_a[0:1, :, :] + 0.587 * gray_img_a[1:2, :, :] + 0.114 * gray_img_a[2:3, :, :]
        gray_img_b = 0.299 * gray_img_b[0:1, :, :] + 0.587 * gray_img_b[1:2, :, :] + 0.114 * gray_img_b[2:3, :, :]
        #
        # print("grayimg_a2: ", type(gray_img_a), np.min(gray_img_a), np.max(gray_img_a), np.median(gray_img_a),
        #       gray_img_a.shape)
        # print("grayimg_b2: ", type(gray_img_b), np.min(gray_img_b), np.max(gray_img_b), np.median(gray_img_b),
        #       gray_img_b.shape)

        gray_img_a -= np.mean(gray_img_a)
        gray_img_b -= np.mean(gray_img_b)
        #
        gray_img_a = torch.from_numpy(gray_img_a).float()
        gray_img_b = torch.from_numpy(gray_img_b).float()

        # print("grayimg_a: ", type(gray_img_a), torch.min(gray_img_a), torch.max(gray_img_a), torch.median(gray_img_a),
        #       gray_img_a.shape)
        # print("grayimg_b: ", type(gray_img_b), torch.min(gray_img_b), torch.max(gray_img_b), torch.median(gray_img_b),
        #       gray_img_b.shape)
        # exit(0)

        result = dict(img1=self.norm(img_a), img2=self.norm(img_b),
                      gray_img1=gray_img_a, gray_img2=gray_img_b,
                      raw_img1=img_a,
                      raw_img2=img_b,
                      )
...
if __name__ == '__main__':
    ...
    db = [data_sources[key] for key in ["D", "W", "A", "S"]]
    # print("db: ", db)
    # db = eval(args.data_loader.replace('`data`', ','.join(db)).replace('\n', ''))
    db = eval(default_dataloader.replace('`data`', ','.join(db)).replace('`R`', str(192)).replace('\n', ''))
    # print("db: ", args.data_loader.replace('`data`', ','.join(db)).replace('`R`', str(args.R)).replace('\n', ''))
    # exit(0)
    ...
    loader = threaded_loader(loader=db, iscuda=True, threads=1, batch_size=1, shuffle=False)
