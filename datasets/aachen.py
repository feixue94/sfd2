...
class AachenPairs_OpticalFlow(AachenImages_DB, PairDataset):
    ...
    def get_mask_filename(self, pair_idx):
        ...
        return os.path.join(self.root_flow, 'mask', '{:s}_{:s}.png'.format(tag_a, tag_b))
    ...
    def get_flow_filename(self, pair_idx):
        ...
        return os.path.join(self.root_flow, 'flow', '{:s}_{:s}.png'.format(tag_a, tag_b))
    ...
    def get_pair(self, idx, output=()):
        ...
        if 'flow' in output or 'aflow' in output:
            ...
            # assert flow.shape[:2] == img1.shape[:2]
            meta['flow'] = flow
            H, W = flow.shape[:2]
        ...
        if 'mask' in output:
            ...
            # assert mask.shape[:2] == img1.shape[:2]
            meta['mask'] = mask
