# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   Hierarchical-Localization -> caps
@IDE    PyCharm
@Author fx221@cam.ac.uk
@Date   21/06/2021 10:57
=================================================='''
# import sys
...
# caps_path = Path(__file__).parent / '../../third_party/caps'
# sys.path.append(str(caps_path))
...
class CAPSNet(nn.Module):
    def __init__(self, pretrained, backbone, coarse_feat_dim, fine_feat_dim, device):
        ...
        self.net = ResUNet(pretrained=pretrained,
                           encoder=backbone,
                           coarse_out_ch=coarse_feat_dim,
                           fine_out_ch=fine_feat_dim).to(self.device)
...
class Caps(BaseModel):
    ...
    def _init(self, conf):
        ...
        self.mean = torch.from_numpy(np.array([0.485, 0.456, 0.406], np.float)).view(-1, 1, 1).float()
        self.std = torch.from_numpy(np.array([0.229, 0.224, 0.225], np.float)).view(-1, 1, 1).float()

        # args = get_args()

        self.sift = cv2.xfeatures2d.SIFT_create(nOctaveLayers=5, contrastThreshold=0.03, edgeThreshold=8)
        device = 'cuda' if torch.cuda.is_available() else 'cpu'
        self.net = CAPSNet(pretrained=True,
                           backbone='resnet50',
                           coarse_feat_dim=128,
                           fine_feat_dim=128,
                           device=device)
        self.net.load_state_dict(torch.load(conf["model_fn"])['state_dict'])
        self.net.eval()

    def _forward(self, data):
        image = data['image']
        # print(image.shape, type(image))
        img_gray = image.cpu().numpy()[0] * 255
        img_gray = np.uint8(img_gray)
        img_gray = np.transpose(img_gray, (1, 2, 0))
        img_gray = cv2.cvtColor(img_gray, cv2.COLOR_BGR2GRAY)
        kpts = self.sift.detect(img_gray)
        scores = np.array([kp.response for kp in kpts], np.float32)
        kpts = np.array([[kp.pt[0], kp.pt[1]] for kp in kpts])
        coord = torch.from_numpy(kpts).float().cuda().unsqueeze(0)

        # img_trans = image
        # img_trans = F.normalize(img_trans, )
        img_trans = image.sub_(self.mean.cuda()).div_(self.std.cuda())
        print('coord: ', coord.shape)
        feat_c, feat_f = self.net.extract_features(img_trans.cuda(), coord)
        descriptors = torch.cat((feat_c, feat_f), -1).squeeze(0).cpu().detach()
        keypoints = coord.squeeze().cpu()
